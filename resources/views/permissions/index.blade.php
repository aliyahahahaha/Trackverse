    ...
    <x-slot name="header">
        <div class="flex items-center justify-between gap-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="font-black text-2xl text-base-content tracking-tight">Permission Management</h2>
                    <p class="text-sm text-base-content/60 font-medium mt-1">Manage access levels and system roles.</p>
                </div>
            </div>
            ...
            <div class="card-body p-0">
                <form action="{{ route('permissions.update', 'bulk_update') }}" method="POST">
                                    ...
                                    @foreach($roles as $role)
                                        <th class="px-6 py-5 text-center">
                                            <div class="inline-flex flex-col items-center">
                                                <span
                                                    class="badge bg-{{ $role['color'] }}/10 text-{{ $role['color'] }} border-none rounded-lg font-black text-[10px] uppercase tracking-widest py-3 px-3">
                                                    {{ $role['name'] }}
                                                </span>
                                            </div>
                                        </th>
                                    ...
                                    <tr
                                        ...
                                        <td class="px-8 py-5">
                                            <div class="flex flex-col gap-1">
                                                <span
                                                    class="font-black text-sm text-base-content/90 group-hover:text-primary transition-colors">{{ $perm['name'] }}</span>
                                                <span
                    ...
                    <div class="p-6 bg-base-100 border-t border-base-content/5 flex justify-end">
                        <button type="submit"
                            class="btn btn-primary shadow-lg shadow-primary/20 font-bold px-8 h-12 rounded-xl text-white">
                            <svg xmlns="http://www.w3.org/2000/svg" class="size-5" viewBox="0 0 24 24" fill="none"
                                stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
